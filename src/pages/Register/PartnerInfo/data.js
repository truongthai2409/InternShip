...
const URL_REGEX = /^(https?:\/\/)?([a-z0-9]+([-_.]{1}[a-z0-9]+)*\.[a-z]{2,5}|(\d{1,3}\.){3}\d{1,3})(:\d{1,5})?(\/.*)?$/i;

const EMAIL_REGEX = /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/;
const PHONE_REGEX = /^[\+]?[(]?[0-9]{3}[)]?[-\s.]?[0-9]{3}[-\s.]?[0-9]{2,6}$/im;
...
export const schema = containerSchema.shape({
  registerPartner: yup.string().default(null).nullable(),
  logo: yup
    .mixed()
    .test(
      "type",
