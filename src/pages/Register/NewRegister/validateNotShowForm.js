...
export const SchemaNotShowForm = () => {
  console.log('Schema');
  return yup.object({
    password: yup
      .string()
    ...
    // companyDTOPhone: yup
    //   .string().when('companyDTOName',(companyDTOName,companyDTOPhone)=>
    //   companyDTOName ? companyDTOPhone.required(' có pass') : companyDTOPhone.required(' k có pass')
    //   ),
    //   // .required('* Bạn phải nhập số điện thoại.')
    //   // .matches(
    //   //   /^((\\+[1-9]{1,4}[ \\-]*)|(\\([0-9]{2,3}\\)[ \\-]*)|([0-9]{2,4})[ \\-]*)*?[0-9]{3,4}?[ \\-]*[0-9]{3,4}?$/,
    ...
    //     '* Bạn đã nhập email không đúng'
    //   ),
    companyDTOTax: yup.string().required('* Vui lòng Chọn mã số thuế công ty'),
    //   // .matches(/^[0-9]+$/, 'Mã thuế không đúng')
    //   // .min(10, '* Nhập đủ mã số  thuế')
    //   // .max(10, '* Nhập đủ mã số  thuế'),
    companyDTOName: yup.string().required('* Vui lòng Chọn tên công ty'),
  });
};
