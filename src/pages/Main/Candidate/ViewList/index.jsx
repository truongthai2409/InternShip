...
const CandidateViewList = () => {
  ...
  const { jobCare, jobCareHavePage} = useSelector(state => state.jobCandidateSlice)
  console.log(jobCare)
    ...
    <>
      <div className="apply__list--container">
        <div className="header__apply">
          ...
          <p className="header__apply--des">Xem lại danh sách những việc làm mà bạn đã lưu trước đó. Ứng tuyển ngay để không bỏ lỡ cơ hội nghề nghiệp dành cho bạn.</p>
        </div>
        <div className="section__apply">
          <span>Bạn đã quan tâm <span>{jobCare.length}</span> việc làm</span>
        </div>
        <Grid
          className="wrapper"
          spacing={{ xs: 2 }}
